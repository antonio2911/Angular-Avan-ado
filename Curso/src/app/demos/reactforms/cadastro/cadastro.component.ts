...
export class CadastroComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    let controleDoEventoBlurs: Observable<any>[] = this.formInputElements.map(
      (formControl: ElementRef) => fromEvent(formControl.nativeElement, 'blur')
    );

    merge(...controleDoEventoBlurs).subscribe(() => {
      this.displayMessage = this.genercValidator.processarMensagem(
        this.cadastroGroup
      );
  ...
  public adicionarUsuario() {
    if (this.cadastroGroup.dirty && this.cadastroGroup.valid) {
      ...
      this.dadosFormJson = 'Não submeteu';
    }
  }
}
